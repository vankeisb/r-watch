...
impl BambooResponse {
    fn to_build_status(&self, server_url: &String) -> Option<BuildStatus> {
        self.results
            ...
            .map(|result| {
                let status = if result.build_state == "Successful" {
                    Status::Green
                } else {
                    Status::Red
                };
